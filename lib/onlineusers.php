...
function OnlineUsers($forum = 0, $update = true)
{
	...
	$rOnlineUsers = Query("select id,name,displayname,sex,powerlevel,lastactivity,lastposttime,minipic from {users} where (lastactivity > {0} or lastposttime > {0})".$forumClause." order by name", time()-300, $forum);
	$onlineUserCt = 0;
