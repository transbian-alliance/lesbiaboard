...
{
	$uid = (int)$uid;
	$rNotifications = Query("SELECT * FROM {notifications} WHERE uid={0}".($scope != "" ? " ".$scope : "")." ORDER BY time DESC", $uid); 
	$notifications = array();
...
{
	Query("INSERT INTO {notifications} (uid, type, title, description, link, linklocation, time) VALUES ({0}, {1}, {2}, {3}, {4}, {5}, {6})",
		(int)$uid, $type, $title, $description, (int)$link, $linkLocation, time()
	);
	return true;
}
...
{
	query("DELETE FROM {notifications} WHERE id={0}", (int)$id);
	return true;
}
