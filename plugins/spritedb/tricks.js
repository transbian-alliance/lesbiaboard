...
function addField(spriteid, thetoken)
{
...
function showsprite(elem, id)
{
	...
	$.get("./", "page=spritedb&e="+id, function(data)
	{
		...
		spriteRow.innerHTML = "<td colspan='2'></td><td colspan='2'><div id='spriteediting' style='display:none;'>"+data+"</div></td>";
		spriteRowID = id;
...
//	alert(elem.rowIndex+" "+id);

/*	var row = elem.parentNode.insertRow(elem.rowIndex+1);

	row.innerHTML = '<td colspan="5"> <iframe src="spriteDB.php?e='+id+'&inline=1" width="100%" height="100%" name="youriframe" frameborder="1" vspace="0" hspace="0" marginwidth="0" marginheight="0" scrolling="yes" noresize></iframe></td>';
*/
}
