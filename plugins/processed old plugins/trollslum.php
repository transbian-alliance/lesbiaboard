<?php
/* Trollslum - Patron Trolls
 * By Kawa
 *
 * External requirements:
 *   None!
 *
 * Homestuck Trolls by Andrew Hussie.
 */

registerPlugin("Trollslum");

function Trollslum_PatronTroll()
{
	global $user;
	if($user['birthday'])
	{
		...
		for($i = count($trolls) - 1; $i >= 0; $i--)
		{
			if($dates[$i] < $bday)
			{
				write("
					<tr>
						<td class=\"cell0\">Patron Troll</td>
						<td class=\"cell1\" title=\"{1}\"><span style=\"color: {2}\">&#x2588;</span> {0}</td>
					</tr>		
", $trolls[$i], $tooltips[$i], $colors[$i]);
				break;
			}
		}
	}	
...
register("profileTable", "Trollslum_PatronTroll");
